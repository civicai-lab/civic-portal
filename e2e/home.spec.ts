...
test.describe("ホームページ", () => {
  ...
  test("Hero セクションが表示される", async ({ page }) => {
    ...
    await expect(heading).toContainText("自治体のDXを、");
  });

  test("サービス一覧を見るボタンが /services にリンクしている", async ({
    page,
  }) => {
    const link = page.getByRole("link", { name: "サービス一覧を見る" });
  ...
  test("注力サービスカードが少なくとも3つ表示される", async ({ page }) => {
    ...
    const cards = section.locator('[class*="card"]').filter({
      has: page.locator("h3"),
    });
    await expect(cards).toHaveCount(5); // Priority S のサービスは5つ
    // 少なくとも3つ以上
  ...
  test("パートナーロゴセクションが表示される", async ({ page }) => {
    // 信頼性セクション（パートナー・認定）
    const partnerSection = page.locator('section[aria-label="信頼のパートナー"]');
